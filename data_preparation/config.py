...
# ================================================================================
# MIGRATION: config.py now lives at data_preparation/config.py (data_preparation is the
# source root). Pin the artifact dir next to this file so generated graphs/word2vec/
# edge_embeds land under data_preparation/artifacts/ regardless of the caller's cwd.
ROOT_ARTIFACT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "artifacts")
# MIGRATION: Ground_Truth is only used by the (unmigrated) detection/evaluation stages,
# not by stages 1-4 that build edge_embeds. Kept as a constant so imports don't break.
# TODO: point this somewhere real if detection is ever migrated.
...
def set_task_paths(cfg):
     ...
     cfg.graph_construction.build_graphs._mimicry_dir = os.path.join(cfg.graph_construction.build_graphs._task_path, "mimicry/")
     # Detection paths
...
def check_args(args):
     # MIGRATION: config/ is now a sibling of this file (data_preparation/config), so look
     # in this file's own dir, not the parent's.
     available_models = os.listdir(os.path.join(os.path.dirname(__file__), "config"))
...
def get_yml_cfg(args):
     ...
     # MIGRATION: config.py moved up one level (src/config.py -> data_preparation/config.py),
     # and config/orthrus.yml now sits at data_preparation/config/. Resolve relative to this
     # file's own dir (.parent) instead of .parent.parent.
