...
Prerequisites for stage 1 (create_db) — server-environment, unchanged from the original:
    ...
    - The target database AND its tables must already exist: run
      `data_preparation/postgres/init-create-databases.sh` (the full-schema variant, not
      `init-create-empty-databases.sh`) once beforehand. The per-dataset scripts here only
      INSERT rows; they create no databases and no tables.
...
def publish_as_dataset(cfg, raw_data_dir, publish_mode, force_publish=False):
    """
    Expose the produced edge_embeds as a new dataset under `raw_data_dir/{DATASET}`,
    so the existing loader (`Custimized`) picks it up with no changes.
    ...
    log(f"[publish] NOTE: training merges this dataset into a single .pt via "
        ...
        f"If an old merged .pt for {cfg.dataset.name} exists (the file passed as "
        f"--data_path, e.g. {cfg.dataset.name.lower()}.pt), delete or rename it before "
        f"training — otherwise training silently keeps using the stale data.")
...
def main(cfg, prep_args):
    ...
    if start <= STAGES.index("create_db"):
        log("=" * 60)
        log("[stage 1/4] create_db: raw logs -> Postgres")
        run_create_database(cfg)
        times["create_db"] = round(time.time() - t0, 2)

    t = time.time()
    if start <= STAGES.index("graphs"):
        log("=" * 60)
    ...
    t = time.time()
    if start <= STAGES.index("featurization"):
        log("=" * 60)
